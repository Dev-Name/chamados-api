...
export function renderCategoryList(): void {
  ...
  for (const c of store.categories) {
    ...
    row.className = "an-row brief" + (c.ativo ? "" : " inact");
    row.innerHTML = `
      <div class="top">
        <span class="nm"><span class="cat-flag" style="background:${esc(c.cor)}" title="${esc(c.cor)}"></span>${esc(c.name)}</span>
        ...
        <div class="a-actions">
          ...
          <button class="ghost sm" data-cat-edit="${c.id}" title="Editar categoria">Editar</button>
          <button type="button" class="icon-btn danger" data-cat-del="${c.id}" title="Excluir categoria">
